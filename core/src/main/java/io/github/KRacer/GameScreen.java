...
// Diagonal movement speed adjustment
        if (moveX1 != 0 && moveY1 != 0) {
            moveX1 *= 0.7f;  // Reduce movement speed to 70% when moving diagonally
            moveY1 *= 0.7f;
        }
...
// Diagonal movement speed adjustment
        if (moveX2 != 0 && moveY2 != 0) {
            moveX2 *= 0.7f;  // Reduce movement speed to 70% when moving diagonally
            moveY2 *= 0.7f;
        }


        // Diagonal movement speed adjustment
        float adjustedSpeed1 = (moveX1 != 0 && moveY1 != 0) ? speed * 0.7f : speed;
        float adjustedSpeed2 = (moveX2 != 0 && moveY2 != 0) ? speed * 0.7f : speed;

        // Apply movement
        motor1Sprite.translateX(moveX1 * adjustedSpeed1 * delta);
        motor1Sprite.translateY(moveY1 * adjustedSpeed1 * delta);
...
//        if (isDimmed || collisionDetected) {
//            return; // Exit the method if dimmed or collision has occurred
//        }
